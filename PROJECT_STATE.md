...
## Межа TransportERP-UA / Taxo

Обидва проєкти розвиваються окремо, але структура TransportERP-UA повинна дозволяти їх подальше злиття без дублювання предметної логіки.

### Taxo — спеціалізований модуль обліку праці водія

Основна ціль Taxo:

- читання та аналіз тахокарт/тахографічних даних;
- облік часу керування, іншої роботи, перерв і відпочинку;
- контроль вимог Положення №340;
- графіки змінності;
- табелі робочого часу водіїв;
- план/факт робочого часу;
- бланки підтвердження діяльності;
- пов’язана з цим аналітика та звітність.

TransportERP-UA **не створює паралельний тахографічний модуль, окремий аналіз №340 або другу систему графіків змінності**, якщо ця функція вже якісно реалізована в Taxo.

### TransportERP-UA — парк, експлуатація, технічна частина і документація

Основна ціль TransportERP-UA:

- підприємство та структурні підрозділи;
- автобуси/транспортні засоби та їх технічна історія;
- водії як кадрово-експлуатаційний довідник;
- маршрути, зупинки, рейси та наряди;
- призначення автобуса й водія;
- випуск на лінію;
- медичний, технічний та диспетчерський контроль;
- шляхові листи;
- фактичний рух, виїзд і повернення;
- пробіг;
- паливо;
- ТО та ремонти;
- документи транспортних засобів, водіїв і підприємства;
- технічна та експлуатаційна звітність.

### Практичне правило сумісності

- де Taxo вже має перевірений модуль (наприклад шляхівка, робоче сховище, тахографічна логіка), TransportERP-UA не створює несумісний дубль;
- спільні предметні об’єкти повинні мати стабільні точки відповідності: водій, транспортний засіб, маршрут, дата роботи, шляховий лист;
- TransportERP-UA зберігає власні UUID; для майбутнього імпорту/злиття Taxo допускається `taxo_legacy_id` або окрема mapping-таблиця, а не заміна UUID на старі integer ID;
- перевірені модулі Taxo можуть тимчасово жити у `transport_erp.taxo_compat`, а ERP-код адаптує до них свої дані;
- `taxo_compat` не є місцем для нового дублюючого функціоналу — це перехідна межа до майбутнього спільного ядра;
- майбутня інтеграція повинна дозволити Taxo читати/повертати фактичний робочий час і тахографічні результати для водія/авто/дати без переписування ERP-модулів парку.
